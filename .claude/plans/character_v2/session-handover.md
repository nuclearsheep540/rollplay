...
## Where we are (updated — ALL PRs 1–14 done; see the PR 10–14 batch section below for the finish-line status + deferred-work list)
...
**⏹ TEST GATE M3 — Runtime sheet: PASSED (user QA'd).**

**PR 9 status (DONE — B.1 + E.1 + E.2, higher-level character creation):**
- **B.1 subclass tracking:** `SubclassEntry` VO + `subclasses` field + `pick_subclass` (one per class, replaces on re-pick) + `character_subclasses` table (mirrors `character_class_entries`) + repo + `SubclassEntryDTO` on `CharacterResponse` + migration `beb9ee0c6021`.
- **E.2 backend (`advancement` draft step + `_apply_advancement`):** picks subclasses (validate-before-mutate); **auto-grants each subclass's always-prepared spells up to the class level** (`source="always_prepared"` — resolves deferral #2's runtime tail, replace-on-save so it doesn't touch class_known/class_prepared); records feats taken *in place of* an ASI (`source="ASI"`, replace-on-save); merges L2+ feature choices onto the class entry's `sub_choices` (choice codes are disjoint across levels, so L1 picks are preserved). No cumulative-ASI math — ability boosts are entered directly on the ability step.
- **E.1/E.2 frontend:** `StartingLevelStep` (pick 1–20; local-only), `AdvancementStep` (per-class walk: subclass radio **with the "normally chosen at level N" hint**, an "ASI or feat" dropdown at ASI levels, and reused `FeatureChoicePicker` for L2+ features), ClassStep prefill (first class → starting level) + total-level indicator. Wizard conditional steps: species → **starting_level** → class → **advancement** (only when total class level > 1) → spells (casters) → background → ability_scores → review. The class→advancement nav rides the existing `stepsRef`-from-fresh mechanism.
- **Scope calls (reasoned deviations from the plan):** (a) NO persisted `starting_level` column — derived from the sum of class levels (already persisted); same behavior, one fewer migration. (b) NO separate `ClassDistributionStep` — reused ClassStep's existing level inputs (+ prefill/indicator); the plan itself said the distribution step "extends the ClassStep visual pattern." (c) ASIs at creation = enter final scores on the ability step + optionally take a feat at ASI levels (avoids cumulative-mutation idempotency issues). Multiclass ASIs are correctly per-class (each class's `asi_levels` vs its own level — this is the real 5e rule).
- **Verified:** 712 backend tests pass (subclass round-trip, multiclass subclasses, always-prepared grant + level-cap, bad-subclass 400, replace-on-save). Frontend lint-clean + compiled. 3-dimension adversarial review with verifiers — **0 real bugs** (every critical/major refuted: `_touch` is only a timestamp so persistence is unaffected; the feature-merge "collision" cannot occur — codes are disjoint; the frontend "hooks-in-a-loop" and early-return findings were self-contradictory, the suggested fixes would have *introduced* bugs). Added a multiclass-subclass test for the one real coverage gap.

**Deferred/not-in-PR9:** feat-prereq hints on the ASI-level feat dropdown (shows all general feats per §3.0; could add the PR8 qualifying/other split as a hint later); rolled-HP-per-level cards (HP uses the ability step + PR8 `computed_hp_max`).

**Next: ⏹ TEST GATE M4 — Higher level + level-up (your manual QA).** Create *at level 5* walking every advancement card (subclass, ASI-or-feat, L2+ features), and confirm always-prepared spells appear for a caster subclass. Then **PR 10 — F.1 (rest) + F.2 (LevelUpModal expansion: subclass step, feature steps, feat description + filter)** — the post-creation level-up flow, which reuses PR 8's `subclass_eligible`/`multiclass_options`/`qualifying_feats` + PR 9's advancement cards. Author + verify per the methodology (`feedback_reference_data_author_verify`).

---

## PR 10–14 batch — the "finish line" push (ALL PRs DONE)

Per the user directive "*get to the finish line ASAP, do the next 4 PRs without stopping*," PRs 10–14 were driven continuously (no per-PR QA checkpoint). **844 backend tests pass; all frontend lint-clean + compiled.** The plan is now feature-complete modulo the explicitly-deferred polish items listed below.

- **PR 10 (F.1 + F.2 — level-up flow):** `LevelUpModal` expanded with a **subclass step** (radio, "normally chosen at level N" hint) and **feat descriptions + qualifying/other split** at ASI levels; consumes PR 8's `subclass_eligible`/`multiclass_options` + PR 9's advancement cards. `preview_level_up` surfaces `subclass_pending` / `feat_details` / `multiclass_options`. `LevelUpCharacter.execute` applies `subclass_choice` (pick_subclass + always-prepared grant capped at class level + SUBCLASS choice-log, all pre-`save()` → atomic). *Deferred:* a dedicated level-up **feature-choice step** (e.g. picking a new metamagic *at* level-up) — the subclass/ASI-feat halves ship; per-level feature picks at level-up are the follow-on.
- **PR 11 (G.3 + Phase H — conditions/exhaustion + party):** `shared/rulesets/conditions.py` (15 SRD conditions + `MAX_EXHAUSTION`); `/conditions` endpoint; `exhaustion_level` (SmallInt col, clamp 0–6 via `set_exhaustion`) on aggregate + runtime PATCH + `ExhaustionTracker` UI. DM party view: `GET /characters/party/{campaign_id}` (DM-or-member auth). *Deferred:* G.2 per-rest **swap UI** (short/long-rest restore buttons); the **party-tab frontend** (backend endpoint is ready); condition-badge **tooltips**.
- **PR 12 (Phase I — cross-service sync):** `api-game/site_client.py::fetch_character_summary` (best-effort GET, `None` on error) + no-auth Docker-internal `GET /characters/{id}/summary` (low-sensitivity snapshot only — follows the `POST /api/campaigns/set-role` internal-call precedent); `seat_change` handler refreshes `player_metadata` on seating.
- **PR 13 (J.1 + J.2 — item + currency catalogues):** parser gained `parse_items` (gear table + `**Name (Cost)**` bold tool/mount entries → 113 items across gear/tools/mounts, `to_code` + `seen`-set dedup) and `parse_currency` (5 fixed coins). `ItemDefinition`/`CurrencyDefinition` models, registry load + `list_items(category=)`/`list_currency`/`get_item`, `/items`?category + `/currency` endpoints, `useEditionItems`/`useEditionCurrency` hooks + tests.
- **PR 14 (J.3–J.5 — inventory + currency storage + runtime panel):** `InventoryItem` VO + `inventory` (child table `character_inventory`, mirrors `character_spells`) + `currency` JSONB col on the aggregate; whole-replace domain methods `replace_currency`/`replace_inventory` (drop-absent-keys semantics **+ `_touch()`**); runtime PATCH whole-map/list replace; `CurrencyInventoryPanel` in the in-game sheet (5-coin editor + inventory list w/ quantity stepper / remove / add-by-name). Migrations `beb9ee0c6021` (subclasses) / `194886c40690` (exhaustion) / `fe1642108b90` (inventory+currency). *Deferred:* wizard **equipment-package picker** + finalize-time expansion — the SRD gives starting equipment as **prose**, so structured packages need authoring first (the storage + runtime edit path is fully done).

**PR 10–14 adversarial review (task `wnpqbiapn`, 10 agents / 3 dimensions):** one genuine bug confirmed (reported twice, same root cause) — the runtime-PATCH `currency`/`inventory` handlers assigned the dataclass field directly, so `updated_at` was **not** refreshed on a currency- or inventory-*only* PATCH (other fields' handlers `_touch()` as a side effect, masking it in mixed updates). **Fixed** by routing both through new whole-replace aggregate methods (`replace_currency`/`replace_inventory`) that assign + `_touch()`, matching every other setter; added 6 regression tests (whole-replace drop-absent semantics + timestamp refresh incl. the empty-collection clear case). Everything else in the review was verified-correct or refuted (storage hydration, atomic level-up, always-prepared level cap, migration integrity, the no-auth summary endpoint's documented risk model).

**Consolidated deferred-work list (all bounded follow-ons; the underlying systems are complete):**
1. Wizard **equipment-package picker** + finalize-time inventory expansion (needs authored structured starting-equipment; SRD is prose). *[PR 14 / Phase J tail]*
2. **G.2 per-rest swap UI** — short/long-rest restore buttons for resource pools (recharge metadata already computed). *[PR 11]*
3. **Party-tab frontend** — the `GET /characters/party/{campaign_id}` endpoint is live; the dashboard tab consuming it isn't built. *[PR 11]*
4. **Level-up feature-choice step** — picking a new per-level feature (e.g. metamagic) *at* level-up; the subclass + ASI-feat halves ship. *[PR 10]*
5. **Condition-badge tooltips** — `/conditions` returns descriptions; the runtime badges don't yet surface them on hover. *[PR 11]*
