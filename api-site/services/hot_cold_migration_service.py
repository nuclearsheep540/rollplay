...
class HotColdMigrationService:
    ...
    async def start_game_session(self, campaign_id: UUID, session_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            game = await self.game_repo.get_by_campaign_id(campaign_id)
            if not game:
            ...
            game = await self.game_repo.update_status(game.id, GameStatus.STARTING)
            if not game:
                raise RuntimeError("Failed to update game status to starting")
            
            # 4. Get campaign data for migration
            campaign_data = await self.campaign_repo.get_campaign_with_game_status(campaign_id)
            if not campaign_data or not campaign_data['can_configure']:
                await self.game_repo.update_status(game.id, GameStatus.INACTIVE)  # Rollback
                raise ValueError("Campaign cannot be configured - game may be active")
            
            # 5. Migrate campaign data to MongoDB hot storage
            hot_storage_data = await self.migration_commands.migrate_to_hot_storage(
                campaign_id, game.id
            )
            
            # 6. Validate migration success
            is_valid = await self.migration_commands.validate_migration_integrity(
            ...
            game = await self.game_repo.update_status(game.id, GameStatus.ACTIVE)
            if not game:
            ...
            try:
                if game:
                    await self.migration_commands.rollback_failed_migration(game.id)
    ...
    async def end_game_session(self, game_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            game = await self.game_repo.update_status(game_id, GameStatus.STOPPING)
            if not game:
                raise RuntimeError("Failed to update game status to stopping")
            
            # 4. Migrate hot storage changes back to PostgreSQL
            migration_data = await self.migration_commands.migrate_to_cold_storage(
                game_id, hot_storage_data
            )
            
            # 5. Update game record with final state
            updated_game = await self.game_repo.update(migration_data['game'])
            if not updated_game:
            ...
            if migration_data['campaign']:
                await self.campaign_repo.update(migration_data['campaign'])
            
            # 7. Validate cold storage migration
            is_valid = await self._validate_cold_storage_migration(game_id, hot_storage_data)
            if not is_valid:
    ...
    async def _validate_cold_storage_migration(self, game_id: UUID, hot_storage_data: Dict[str, Any]) -> bool:
