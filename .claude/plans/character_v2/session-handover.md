...
**Next action: PR 5 — Invocations + Metamagic catalogues** (A.7/A.8), then wire the deferred Warlock Invocations/Pact Boon + Sorcerer Metamagic choices into `class_choices.json` (deferral #3). Spell content (#1), spellcasting tables (A.6), and subclass always-prepared spells (#2) are all DONE. Author + verify per the methodology (`feedback_reference_data_author_verify`).
