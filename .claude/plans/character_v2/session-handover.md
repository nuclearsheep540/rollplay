...
**Next action: PR 8 — B.6 HP-recompute-on-CON-change + C.3 (compute_hp_max) + C.5 subclass eligibility + Phase D guidance surfacing** (size S, per the plan). Then ⏹ **TEST GATE M3 — Runtime sheet** (take a character into a live session; edit HP/slots/resources/conditions; confirm derived + HP-on-CON recompute). Author + verify per the methodology (`feedback_reference_data_author_verify`).
