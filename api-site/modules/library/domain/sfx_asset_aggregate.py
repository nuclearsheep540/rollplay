...
"""
SfxAsset Aggregate - Domain model for SFX assets with playback configuration
...
@dataclass
class SfxAsset(MediaAssetAggregate):
    """
    SfxAsset domain aggregate.
    ...
    loads this effect into a game, these values are used as starting config.
    """
    ...
    def create(
        ...
        """
        Factory method to create a new SFX asset.

        Forces asset_type to SFX. Validates content_type is audio.
        """
        valid_audio_types = {"audio/mpeg", "audio/wav", "audio/ogg"}
        if content_type not in valid_audio_types:
            raise ValueError(f"Invalid content_type for sfx: {content_type}")
    ...
    def from_base(
        ...
        """
        Promote a base MediaAssetAggregate to SfxAsset.
