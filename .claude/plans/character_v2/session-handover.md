...
**Next action: PR 7 — A.9 weapons/armor + B.3 resource pools + C.4 AC methods + G.1 runtime sheet** (per the plan's PR table); or the subclass-selection flow (PR 9/10) to light up always-prepared + the Druid land pick. Author + verify per the methodology (`feedback_reference_data_author_verify`).
