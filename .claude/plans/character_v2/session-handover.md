...
**Next action: PR 6 — spell selection** (spell picks on the aggregate, multiclass slot math, SpellsStep wizard) + the remaining deferral #3 catalogue choices (Mystic Arcanum, Wizard spellbook, Bard Magical Discoveries) and the Druid Circle of the Land land single_pick (#2 tail). Author + verify per the methodology (`feedback_reference_data_author_verify`).
