...
36. **The token select area biases avatar cover-positioning on character surfaces** (settled with Matt 2026-08-17). Dashboard character cards and the avatar pane render `bg-cover bg-center` today — geometric center, which happily cuts the face off a portrait in a horizontal card. Change: where an avatar has a `token` focal area, the cover *position* biases toward it via the percentage focal-point mapping (`background-position: cx% cy%`, focal center as a fraction of the image). Not a hard crop — the image still cover-fills; the bias only decides which slice of the overflow axis survives. Three states, one fallback chain: area → focal position; avatar without area (pre-feature uploads) → center, byte-identical to today; no avatar → `/heroes.png`, center. Simple mapping deliberately chosen over a clamp-the-square-into-view variant: percentages cannot overshoot (0%/100% pin at image edges — no gaps possible), the focal *center* is always in view by construction, and image-space maps monotonically to card-space so off-center portraits keep their framing. Bounded losses accepted: an edge-hugging area can hang part of its square off the card (center stays visible), and an area larger than the card can show at cover zoom centers on the area (no positioning could do better). Tighten to the clamped variant only if QA shows edge crops losing too much.
37. **Character index strip redesign** (briefed by Matt 2026-08-17). The dashboard character index becomes a full-bleed strip of 35° parallelogram cards — the app's slant motif (workshop tool tiles, avatar pane wedge) promoted to a true tiling. Specifics settled:
    - **Parallelogram, both edges** at 35° from vertical (descending edge drifts left, matching the existing wedge family). Right-edge-only cannot fit a full-height 35° slant on a portrait card (tan 35° ≈ 0.7 × height ≈ 124% of a 9:16 width); with both edges slanted the angle is free and neighbours tile with negative margins. The DOM box is wider than the visible band (box = stride + tan35°×height); fixed aspect ⇒ static clip-path percentage constants, tool-nav style.
    - **First/last edges**: ~~all cards identical with an angled lead-in void~~ **QA overturned 2026-08-18**: at full-bleed heights the void is tan 35° × H ≈ 860px of dead space — read as broken. The first card (and the create-card when it leads an empty strip) square-caps its left edge (`STRIP_FIRST_CLIP`) and drops the seam shadow (which exists to hug the diagonal that card lacks). The strip's tail keeps its slant.
    - **Rest state: greyscale by default; hover = color + scale + z-pop.** Chosen over a spotlight variant for being pure per-card CSS (no group coordination, no render conditions). `saturate(0)` sits on the avatar image layer only, so the name and In Game badge stay crisp. The row needs vertical overflow headroom for the hover scale, and the hovered card takes a z-index bump to pop over neighbours.
    - **Diagonal seams carry the perpendicular inner shadow** (the 125°-for-35° gradient from `WorkshopToolNav`), giving the overlapping-cards depth.
    - **Meta reduced to the name only**, top of card, prouder than today, no backplate. QA judgement: try bare text first; fallback is a whisper of text-shadow or a thin top scrim — the band does not return. Name's left inset follows the top edge's slant; In Game badge keeps the top-right (the widest point).
    - **Kept**: horizontal scroll + the wheel→horizontal converter, and the create-card — restyled as a slanted knocked-out member of the strip. Continuity bonus: clicking it lands on the wizard whose 33vw avatar pane opens with the same wedge motif.
38. **Index transitions: in-page choreography now, cross-route flight as stretch.** Index→view is a same-tree state swap (like campaigns' `selectedCampaignId`) — GSAP (already a dependency) animates the card→drawer choreography: avatar slides left, meta drawers in from the right. Create/edit push a route (`/character/create`) and dismount — accepted as a **cut**, softened by the wizard's layout mirroring the drawer (avatar pane left, content right, same slant family). Stretch, not core: a shared-overlay FLIP flight in the persistent `(authenticated)` layout (survives child navigation) flying the avatar card→pane across the push. Next's View Transitions API rejected for now (experimental, no Firefox).
39. **Session start re-stamps placed PC tokens' `image_asset_id` from the current roster.** Decision 24 keeps PC tokens alive across sessions via the paused board, which would otherwise freeze the avatar they were placed with. Invariant by construction: *a PC token's image always derives from the character's current avatar as of session start.* Avatar cleared between sessions → `None` → color disc.
...
- **Re-stamp** (decision 39): after the three-way merge in `_restore_map_token_state`, overwrite each `kind == "pc"` token's `image_asset_id` with its owner's current `avatar_asset_id` (roster lookup by `owner_user_id`; owner absent from roster → leave untouched). Runs before `_build_token_images` so the board scan sees fresh ids.
...
### 3.6 Focal-biased avatar positioning on character surfaces (decision 36)

- **Response field:** `CharacterResponse.avatar_focal_area: Optional[FocalArea-shaped dict]` — read off the eager-loaded avatar asset in `_to_character_response` (the same hop that populates `avatar_s3_key`; no extra query). api-site-only — NOT a shared_contracts change; api-game never sees it.
- **Shared helper:** one small shared hook (`useImageFocalPosition(url, area)` or similar) that probes `naturalWidth/Height` exactly as `TokenAvatarDisc` does, converts the native-px area center to `background-position` percentages, and returns undefined when `area` is null (caller falls back to `bg-center`). Until the probe resolves, render center — same progressive settle the token discs already have.
- **Surfaces:** dashboard character card (`CharacterManager.js` wedge layer) and `CharacterAvatarPane` (wizard, drawer, sheet). Anywhere else that cover-fits `avatar_url` adopts the same helper when touched.
- **Extended 2026-08-18** (QA: the campaign drawer wasn't biasing). The two remaining wedge surfaces adopted it: the campaign drawer's party seats (`CampaignManager.js` → extracted `PartyMemberCard`) and `CharacterSelectionModal` (→ extracted `CharacterChoiceCard`). Both were inline JSX inside a `.map()`, so both needed extracting for the same reason `CharacterStripCard` exists — the bias is a hook. The party seats also needed the data: `character_avatar_focal_area` added to `GetCampaignMembers`' dict and `CampaignMemberResponse` (`avatar_asset` is already `lazy="joined"`, so no extra query). `CharacterSelectionModal` needed frontend only — it reads `CharacterResponse`, which already carried the field. Verified: a directional gradient layered under the portrait is unaffected by `background-position`, because a gradient has no intrinsic size and `cover` fits it exactly to the box — so the single-value `backgroundPosition` the character card uses is safe on the wedges too.
- Cards/panes never read `avatar_asset_id` for this — the area rides the character response, so no per-card asset fetches.
...
| **13 — Character index redesign + focal positioning** | §3.6 focal-biased positioning (`avatar_focal_area` response field + shared position hook + card/pane adoption); decision 37 strip redesign (parallelogram tiling, greyscale-at-rest hover, name-only, create-card in-strip); decision 38 in-page index→view GSAP choreography + layout-mirrored cut for create/edit. Stretch (not built): shared-overlay FLIP flight. Implemented 2026-08-17, awaiting QA. | feature branch |
...
- No clamp-the-square-into-view position mapping — simple percentage mapping only, revisit on QA evidence (decision 36).
- No hard crop on cards/panes — cover-fill stays; only the position biases (decision 36).
...
- **PR 13:** portrait avatar on the horizontal dashboard card keeps the face in view; landscape avatar on the portrait pane likewise (one overflow axis each); edge-hugging select area pins near the card edge with the face center visible; avatar with no area renders byte-identical to today; default `/heroes.png` unchanged; probe settle causes no layout shift (position-only change).
...
1. ~~Verify at PR 12 implementation: `player_view_changed` for pc `place`~~ **RESOLVED 2026-08-17**: `place` sets `player_view_changed = not hidden` (`websocket_events.py:1845-1846`) and pc tokens can never be hidden (contract validator) — the image-ref piggyback always fires for pc placement. Nothing in api-game strips `image_asset_id` from place payloads (`CONFIGURABLE_TOKEN_FIELDS` gates configure only). Zero api-game changes confirmed.
