...
| **PR 8** ✅ | B.6 + C.3 + C.5 + Phase D rest | **DONE.** `compute_hp_max` (+`DerivedStats.computed_hp_max`); `apply_asi(ruleset=)` CON-delta HP recompute (non-destructive, via new `ability_modifier`); `can_pick_subclass` + `can_add_class`; `LevelUpPreview.subclass_eligible`/`multiclass_options` (guidance, never gates). Backend-only (modal UI is PR 10). 705 tests; review found 0 real bugs. | S |
...
| **PR 9** ✅ | E.1 + E.2 (+ B.1) | **DONE.** B.1 subclass tracking (`character_subclasses` + `pick_subclass`); `advancement` draft step + `_apply_advancement` (subclass + auto-grant always-prepared spells + ASI-level feats + L2+ features); StartingLevelStep + AdvancementStep (subclass/feat/feature cards + subclass hint) + ClassStep prefill/indicator; conditional wizard steps. Reasoned deviations: no persisted starting_level (derive from class levels), reused ClassStep for distribution, ASIs entered on ability step. 712 tests; review found 0 real bugs. | M |
| **PR 10** ✅ | F.1 + F.2 | **DONE.** Level-up modal gains a **subclass step** (offered when the gained level unlocks one; `subclass_pending` in preview; `LevelUpCharacter` records it + grants always-prepared spells) + **feat descriptions** (`feat_details` in preview) atop the existing qualifying/other prereq split. 713 tests. Deferred (ASAP): level-up feature-choice step (metamagic-at-level-up). | M |
...
| **PR 11** ✅ | G.3 + Phase H | **DONE (core).** G.3 conditions (`conditions.py` + `/editions/{code}/conditions` endpoint) + `exhaustion_level` (aggregate/column/migration/runtime PATCH + runtime-sheet stepper). Phase H DM party endpoint `GET /characters/party/{campaign_id}` (DM-or-member auth). 715 tests. **Deferred (ASAP):** G.2 per-rest swap UI; the party-tab frontend (backend ready, reuses read-only CharacterSheet); condition-badge tooltips. | M |
| **PR 12** ✅ | Phase I | **DONE.** `fetch_character_summary` pull helper in api-game `site_client`; Docker-internal `GET /api/characters/{id}/summary` (no-auth, snapshot fields only — follows the set-role precedent); `seat_change` handler best-effort-refreshes player_metadata so level-up/HP/AC flow into the live session. | S |
| | | **⏹ TEST GATE M5 — Multiplayer + DM.** DM party view; per-rest swaps; changes reflected in the live api-game session. | |
| **PR 13** ✅ | J.1 + J.2 | **DONE.** `ItemDefinition` (flat, category-discriminated) + parser for gear/tools/mounts (113 items: 82 gear/23 tools/8 mounts) + `CurrencyDefinition` (5 coins). Registry + `/editions/{code}/items`?category + `/currency` endpoints + `useEditionItems`/`useEditionCurrency` hooks + tests. | M |
| **PR 14** ✅ | J.3 + J.4 + J.5 | **DONE (core).** `InventoryItem` VO + `inventory` child table + `currency` JSONB column on the aggregate; repo + `CharacterResponse`/`RuntimePatchRequest` (whole-list/map replace, no enforcement — negatives + qty-0 allowed) + migration; runtime **Currency + Inventory panel** (edit coins, item quantity/notes/add/remove). 838 tests. **Deferred (ASAP):** wizard equipment-package picker + finalize-time expansion (needs structured starting-equipment; SRD gives it as prose). | M |
