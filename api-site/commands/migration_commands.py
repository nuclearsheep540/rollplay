...
class MigrationCommands:
    ...
    async def validate_migration_integrity(self, game_id: UUID, hot_storage_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            campaign = await self.campaign_repo.get_by_id(game.campaign_id)
            if not campaign:
                return False
    ...
    async def rollback_failed_migration(self, game_id: UUID) -> bool:
        ...
        try:
            ...
            game = await self.game_repo.update_status(game_id, GameStatus.INACTIVE)
            return game is not None
            
        except Exception:
            return False
    
    async def cleanup_orphaned_hot_storage(self, game_id: UUID) -> bool:
