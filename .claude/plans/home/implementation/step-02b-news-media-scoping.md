...
| 13 | `list_objects` reads only the first 1000 keys, so a restore could silently omit documents | **Deferred by Matt (2026-09-01), and the reasoning is sound**: S3 holds the durable copy, so a truncated restore is incomplete RECOVERY, not loss — my "partial data loss" framing was overstated. Only three callers list S3, two are folder-scoped and cannot approach the cap, and the library never lists S3 at all (rows in PostgreSQL). Actioned instead: the docstring corrected (it claimed a caller "cannot present a partial listing as a complete one", which is exactly what truncation does), and **truncation now logs at ERROR** — so the day it starts happening is visible rather than silent. Pagination itself remains open |
